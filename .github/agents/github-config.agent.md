...
4. coding-agent-dispatcher detects new issue in github-config
  ...
  └─→ Agent commits to the existing dispatcher-created branch/PR
...
**CRITICAL:** The dispatcher has already created and linked a working branch and PR for this issue assignment. In this context, do **not** create a new branch or PR. Commit changes to the existing branch and update the existing PR.
...
4. **Use the existing dispatcher-created branch/PR context** and commit directly to that branch (no branch creation)
5. **Commit** the updated `terraform/terraform.tfvars` with message: `feat(repo): Add workload repository — {repository_name}`
6. **Update the existing dispatcher-created PR (best effort)** with:
...
1. **Determine Target** - Ask user if unclear (current repo, dedicated IaC repo, or specified)
  ...
  - **Cloud issue-assigned context:** Reuse the existing dispatcher-created branch and PR
