...
4. coding-agent-dispatcher detects new issue in github-config
  ...
  └─→ Agent commits to the existing assignment-linked branch/PR
...
**CRITICAL:** Issue assignment may already create/link a working branch and PR. In this context, do **not** create another branch or PR when one already exists. Commit changes to the existing branch and update the existing PR.

**CRITICAL (Issue identity):** Treat the currently assigned GitHub issue as the single source of truth for the triggering issue number. Do **not** infer the triggering issue number from free-text lines like "Generated by ... for issue #..." if they conflict. Use the `Source Issue` field from the table only for cross-repo reference (for example, `nathlan/alz-subscriptions#43`).

**CRITICAL (Commit hygiene):** Do not create "initial plan" or progress-only commits. Only commit when repository files have actual content changes.

**Issue identity rules (cloud issue-assigned):**
- Treat the currently assigned GitHub issue as the authoritative triggering issue.
- Treat `Source Issue` in the configuration table as an external cross-reference (for example, `nathlan/alz-subscriptions#<n>`), not the triggering issue in this repository.
- If issue numbers in free text conflict, prefer assignment context and current issue metadata over embedded prose.

**Permission-aware behavior:**
- Some integrations can push commits but cannot create/update PRs or post issue comments (`403 Resource not accessible by integration`).
- If PR/comment API operations return `403`, do not repeatedly retry. Continue by committing code to the existing branch and report the limitation clearly in the final summary.
...
4. **Use the existing assignment-linked branch/PR context** and commit directly to that branch (no branch creation)
5. **Commit** the updated `terraform/terraform.tfvars` with message: `feat(repo): Add workload repository — {repository_name}`
6. **Update the existing assignment-linked PR (best effort)** with:
...
7. **Post a comment on the triggering issue** with a link to the existing PR (best effort)

**If PR linkage is unclear:**
- First, check for an existing PR associated with the current working branch.
- If a PR exists, reuse it.
- Only create a new PR in cloud issue-assigned context when no existing PR is found.

**Permission fallback (required):** If PR/issue metadata operations return `403 Resource not accessible by integration`, do **not** fail the task and do **not** retry repeatedly. Treat code commit/push as the primary deliverable, then continue and finish with a concise summary of what was skipped due to permissions.

  - If update/comment operations are blocked by permissions (`403`), skip those operations and include this explicitly in the final task summary.
...
1. **Determine Target** - Ask user if unclear (current repo, dedicated IaC repo, or specified)
  ...
  - **Cloud issue-assigned context:** Reuse the existing assignment-linked branch and PR
  - **Standalone/ad-hoc context:** Create a new branch and draft PR

2. **Branch Handling**

  - **Cloud issue-assigned context:** Do not create a branch; commit to the existing branch
  - **Standalone/ad-hoc context:** Create branch with descriptive name: `terraform/github-config-<description>`, use GitHub MCP `create_branch`
...
4. **PR Handling**
  ...
  - **Cloud issue-assigned context:** Update the existing PR (title/body/labels if needed, best effort)
  - **Standalone/ad-hoc context:** Create draft PR including:
  ...
  - **Standalone/ad-hoc only**: `draft: true`

  **Permission fallback:** If PR update or issue comment APIs return `403 Resource not accessible by integration`, do not fail the task if code commits were pushed successfully. Record the limitation in the summary and request manual PR/issue metadata updates.
...
Agent: Discover repos → Found 47 → Ask: All or filtered? What protection level? → User specifies → Confirm scope → Generate → Validate → Update existing PR (cloud issue-assigned) or create draft PR (standalone/ad-hoc) → Provide summary
...
- [ ] PR updated (cloud issue-assigned) or draft PR created (standalone/ad-hoc)
- [ ] No progress-only commits (each commit includes file changes)
